...
public class DetteController {
    ...
    public ResponseEntity<ApiResponse<Map<String,Object>>> rembourser(
        ...
        try {
            ...
            // Cet endpoint REST direct n'a pas d'operationUuid propre à
            // une opération multi-mode (voir MultiModeService pour le
            // chemin qui en a un) — la valeur null fait retomber
            // rembourser() sur sa clé de repli déterministe.
            String operationUuid = body.get("operationUuid") != null
                    ? body.get("operationUuid").toString() : null;
            return ResponseEntity.ok(ApiResponse.ok(
                    "Remboursement enregistré",
                    detteService.rembourser(uuid, montant, auth.getName(), operationUuid)
            ));
        } catch (Exception e) {
            return ResponseEntity.ok(ApiResponse.error(e.getMessage()));
        }
